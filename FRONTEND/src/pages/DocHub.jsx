...
export default function DocHub() {
  ...
  const renderA4Document = () => {
    ...
    return (
      <div className="w-[210mm] min-h-[297mm] bg-white text-black p-[25.4mm] shadow-lg mx-auto font-serif text-[12pt] leading-normal mb-10">
        {draftResult.title && (
  ...
  return (
    <PageContainer>
      <div className="flex flex-col gap-6 text-left max-w-5xl mx-auto mt-4 pb-20">
        ...
        {/* Header */}
        <div>
          <span className="text-[10px] font-bold text-indigo-800 uppercase tracking-widest bg-indigo-500/10 px-3 py-1.5 rounded-full border border-indigo-500/10">
            NYAAY AI Engine
          </span>
          <h1 className="text-3xl font-semibold tracking-tighter mt-4">Document Drafting</h1>
        ...
        {step === 1 && (
          <Card className="p-8">
            ...
            <textarea
              className="w-full h-48 p-4 border border-zinc-300 rounded focus:outline-none focus:border-indigo-500 resize-none text-sm"
        ...
        {(step === 2 || (step === 4 && isGenerating)) && (
          <Card className="p-16 flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-[#6B6B6B] mt-2 text-center max-w-md">
        ...
        {step === 3 && (
          <Card className="p-8">
            <h2 className="text-xl font-bold mb-2">Missing Information Required</h2>
            ...
            <div className="space-y-4">
              {missingInfo.fields.map(field => (
                <div key={field} className="flex flex-col gap-1">
                  ...
                  <input
                    type="text"
                    className="p-2 border border-zinc-300 rounded focus:border-indigo-500 focus:outline-none text-sm w-full max-w-md"
        ...
        {step === 4 && !isGenerating && draftResult && (
          <div className="flex flex-col gap-6">
            ...
            <div className="sticky top-4 z-10 bg-white/90 backdrop-blur-sm p-4 border border-zinc-200 rounded-lg shadow-sm flex flex-col gap-3">
              <div className="flex flex-wrap items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => window.print()} className="!py-1.5 !px-3 text-xs flex items-center gap-1">
                    <span className="material-symbols-outlined text-[16px]">print</span> Print
                  </Button>
                  <Button onClick={handleDownloadPdf} className="!py-1.5 !px-3 text-xs bg-red-600 hover:bg-red-700 text-white flex items-center gap-1">
                    <span className="material-symbols-outlined text-[16px]">picture_as_pdf</span> PDF
                  </Button>
                  <Button onClick={handleDownloadDocx} className="!py-1.5 !px-3 text-xs bg-blue-600 hover:bg-blue-700 text-white flex items-center gap-1">
                    <span className="material-symbols-outlined text-[16px]">description</span> DOCX
                  </Button>
                  <div className="w-px h-6 bg-zinc-300 mx-1 self-center"></div>
                  <Button variant="outline" onClick={() => setIsEditing(!isEditing)} className={`!py-1.5 !px-3 text-xs ${isEditing ? 'bg-indigo-50 text-indigo-700 border-indigo-500' : ''}`}>
                    <span className="material-symbols-outlined text-[16px] mr-1 align-middle">edit</span>
                    Edit Draft
                  </Button>
                  <Button variant="outline" onClick={handleCopyText} className="!py-1.5 !px-3 text-xs">Copy Text</Button>
                  <Button variant="outline" onClick={() => setStep(1)} className="!py-1.5 !px-3 text-xs">Start Over</Button>
                </div>
              </div>
              ...
              {isEditing && (
                <div className="pt-3 border-t border-zinc-200 flex gap-2">
                  <input 
                    type="text" 
                    className="flex-1 p-2 border border-zinc-300 rounded text-sm focus:outline-none focus:border-indigo-500" 
            ...
            {/* A4 Document Preview */}
            <div className="overflow-x-auto bg-zinc-100 p-8 rounded-lg shadow-inner flex justify-center">
              {renderA4Document()}
            </div>
          </div>
        )}
